...
class ThUsulanTenderService
{
            ...
            if ($validator->fails()) {
                ...
                foreach ($docs as $i => $doc) {
                    ...
                    $docsValidator = Validator::make($doc, [
                        'nama_berkas' => 'required_if:tmjenistenderdoc_id,0',
                        'berkas' => 'required|file|mimes:pdf|max:250000',
    ...
    public function update_service($request, $tender_id)
    {
        ...
        foreach ($usulanTenderDetails as $index => $detail) {
            ...
            foreach ($docs as $i => $doc) {
                if (isset($doc['id'])) {
                    $model_doc = ThUsulanTenderDetailDoc::find($doc['id']);
                    if (isset($doc['is_del']) && $doc['is_del'] && $doc["tmjenistenderdoc_id"] == 0) {
                        $model_doc->delete();
